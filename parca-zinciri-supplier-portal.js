...
(function () {
  ...
  function seedDocuments() {
    return [
      { id: "D1", name: "Vergi levhası", status: "Onaylandı", updatedAt: "2026-06-12" },
      { id: "D2", name: "İmza sirküleri", status: "İnceleniyor", updatedAt: "2026-07-20" },
      { id: "D3", name: "Faaliyet belgesi", status: "Yüklendi", updatedAt: "2026-07-18" },
      { id: "D4", name: "Marka yetki belgesi", status: "Eksik", updatedAt: "—" },
      { id: "D5", name: "Kalite belgeleri", status: "Güncelleme Gerekli", updatedAt: "2026-05-02" }
    ];
  }
...
.field input,.field select,.field textarea{
  ...
  border:1px solid var(--line-2);background:#141414;color:var(--text);
}
...
.chip{
  ...
  border:1px solid var(--line-2);font-family:var(--mono);font-size:10px;letter-spacing:.08em;text-transform:uppercase;color:var(--mid);background:rgba(255,255,255,.02)
}
.chip::before{content:"";width:6px;height:6px;border-radius:50%;background:currentColor}
.chip.urgent,.chip.acil{color:#FFB088;border-color:rgba(255,107,0,.35)}
.chip.ok,.chip.onaylandi,.chip.kabul,.chip.teslim{color:#9AF0C8;border-color:rgba(61,220,151,.35)}
.chip.warn,.chip.inceleniyor,.chip.revizyon,.chip.hazirlaniyor{color:#FFE08A;border-color:rgba(245,197,66,.35)}
...
.upload-zone{
  ...
  background:#141414;display:flex;flex-direction:column;gap:6px;justify-content:center;cursor:pointer
}
...
.topbar{
  ...
  padding:0 20px;border-bottom:1px solid var(--line);background:rgba(15,15,15,.96);backdrop-filter:blur(8px)
}
...
.badge{
  ...
  background:var(--accent);color:#111;font-size:10px;font-weight:700;display:grid;place-items:center
}
...
.main{overflow:auto;padding:20px;background:
  linear-gradient(180deg, rgba(255,107,0,.03), transparent 180px),
  var(--void)
}
...
.req-card{
  border:1px solid var(--line);border-radius:10px;padding:14px;background:#151515;margin-bottom:10px
}
...
.toolbar input,.toolbar select{
  min-height:40px;padding:8px 10px;border-radius:8px;border:1px solid var(--line-2);background:#141414;color:var(--text)
}
...
table.data th{
  ...
  background:#151515;position:sticky;top:0;z-index:1;white-space:nowrap
}
...
.tech-viz{
  ...
  background:
    ...
    linear-gradient(135deg,#151515,#1c1c1c);
  position:relative;overflow:hidden
}
...
.calc-box{display:grid;gap:8px;padding:12px;border-radius:8px;background:#141414;border:1px solid var(--line);margin:12px 0;font-size:13px}
...
.notif-panel{
  ...
  box-shadow:0 16px 50px rgba(0,0,0,.45);max-height:70vh;overflow:auto
}
...
.progress{height:8px;border-radius:999px;background:#222;overflow:hidden;margin:10px 0 18px}
.progress > i{display:block;height:100%;background:var(--accent);width:0}
.doc-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(220px,1fr));gap:12px}
.doc-card{border:1px solid var(--line);border-radius:10px;padding:14px;background:#151515}
.map-grid{display:grid;grid-template-columns:1fr 1fr;gap:8px;margin:12px 0}
.map-grid select{min-height:40px;width:100%;border-radius:8px;border:1px solid var(--line-2);background:#141414;padding:8px}
...
@media (max-width:768px){
  ...
  .bottom-nav{
    ...
    background:#141414;padding:6px 4px calc(6px + env(safe-area-inset-bottom))
  }
  .bottom-nav button{
...
`;
  ...
  class ParcaZinciriSupplierPortal extends HTMLElement {
    ...
    _chipClass(label) {
      ...
      if (/eksik|red|sorun|pasif|doldu/.test(l)) return "chip danger";
      ...
      if (action === "quick") {
        ...
        if (qk === "quote") {
          var first = s.requests.find(function (r) {
            return r.quoteStatus !== "Teklif Gönderildi";
          });
    ...
    _renderOverview() {
      ...
      return (
        ...
        esc(greeting()) +
        "</h2>" +
        '<p class="muted" style="margin-top:6px">Firma operasyonlarınıza genel bakış</p>' +
        '<div class="stats">' +
    ...
    _requestCard(r, compact) {
      return (
        ...
        esc(r.id) +
        '">Detayı Aç</button>' +
        '<button type="button" class="btn sm primary" data-action="quote-from-request" data-id="' +
        esc(r.id) +
    ...
    _renderRequests() {
      ...
      return (
        ...
        ["Talep", "Parça", "OEM", "Araç", "Yıl", "Adet", "Tercih", "Şehir", "Aciliyet", "Son tarih", "Teklif"]
          .map(function (h) {
            return "<th>" + h + "</th>";
          })
          .join("") +
    ...
    _renderDrawer() {
      ...
      if (d.type === "request") {
        ...
        footer =
          '<button type="button" class="btn primary block" data-action="quote-from-request" data-id="' +
          esc(r.id) +
